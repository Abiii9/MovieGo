...
def user_on_product_newpage(context):
    ...
    open_url = urljoin(base_url,'product_new/1/2')
    context.browser.get(open_url)


@when(u'we fill in the form')
def user_fills_form(context):
    bookingdate_textfield = context.browser.find_element('name', 'booking_date')
    bookingdate_textfield.send_keys('2023-08-09')
    bookingtime_textfield = context.browser.find_element('name','booking_time')
    bookingtime_textfield.send_keys('22:01')
