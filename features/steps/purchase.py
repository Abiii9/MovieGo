...
@given(u'user wants to purchase the product and clicks on purchase button')
def user_clicks_on_purchase(context):
    ...
    context.browser.find_element(By.CLASS_NAME,'purchase_btn').click()


@when(u'user enters his card details')
def user_enters_details_and_submit(context):
    cardtypefield = context.browser.find_element('name','payment')
    cardtypefield.send_keys('visa')
    cardnumfield = context.browser.find_element('name','total')
    cardnumfield.send_keys('123456789123')
    context.browser.find_element('name','submit').click()
