...
@given(u'we want to view details about a movie')
def user_on_movies_page(context):
    use_fixture(setup_model_objects, context)
    base_url = urllib.request.url2pathname(context.test_case.live_server_url)
    open_url = urljoin(base_url,'movies/')
    context.browser.get(open_url)

@when(u'we search for a movie and click movie image')
def user_searches_for_movie(context):
    moviename_textfield = context.browser.find_element('name', 'movie')
    moviename_textfield.send_keys('Toy Story')
    context.browser.find_element('name','submit').click()
    base_url = urllib.request.url2pathname(context.test_case.live_server_url)
    open_url = urljoin(base_url,'movie_details/1')
    context.browser.get(open_url)


@then(u'we can see the details about the movie')
def user_sees_movie_details(context):
    assert "Andy's toys" in context.browser.page_source
