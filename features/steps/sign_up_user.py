...
@given(u'user is on the home page and clicks signup')
def user_is_on_homepage(context):
    use_fixture(setup_model_objects, context)
    base_url = urllib.request.url2pathname(context.test_case.live_server_url)
    open_url = urljoin(base_url,'/')
    context.browser.get(open_url)
    context.browser.find_element(By.CLASS_NAME, "signup_btn").click()


@when(u'user enters details and submits')
def user_enters_signup_details(context):
    usernamefield = context.browser.find_element('name','username')
    usernamefield.send_keys('JaneMcGill')
    emailfield = context.browser.find_element('name','email')
    emailfield.send_keys('janemcgill@somemail.com')
    passwordfield = context.browser.find_element('name','password1')
    passwordfield.send_keys('Password@123')
    confirmpasswordfield = context.browser.find_element('name','password2')
    confirmpasswordfield.send_keys('Password@123')
    addressfield = context.browser.find_element('name','address')
    addressfield.send_keys('123 Qatar Junction, Qatar')
    firstnamefield = context.browser.find_element('name','first_name')
    firstnamefield.send_keys('Jane')
    lastnamefield = context.browser.find_element('name','last_name')
    lastnamefield.send_keys('McGill')
    context.browser.find_element('name','Sign Up').click()


@then(u'user is able to sign up to be a customer of the application')
def user_succeeds_signup(context):
    print(context.browser.page_source)
    assert 'Welcome back, Jane' in context.browser.page_source
