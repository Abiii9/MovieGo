...
@given(u'we want to view details about the available zones')
def user_on_movie_details_page(context):
...
@when(u'we add a movie to cart')
def step_impl(context):
    context.browser.find_element(By.CLASS_NAME, "btn").click()


@then(u'we can see the details about zones available')
def step_impl(context):
    assert "Resolution: 2K" in context.browser.page_source